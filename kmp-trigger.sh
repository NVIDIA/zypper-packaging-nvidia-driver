...
cat >  /usr/lib/tmpfiles.d/nvidia-logind-acl-trick-G04.conf << EOF
...
for dev in $(ls -d /sys/bus/pci/devices/*); do 
  ...
  if [ "$vendorid" == "0x10de" ]; then 
    ...
    if [ "$classid" == "0x0300" -o "$classid" == "0x0302" ]; then 
      ...
      echo "L /run/udev/static_node-tags/uaccess/nvidia${devid} - - - - /dev/nvidia${devid}" >> /usr/lib/tmpfiles.d/nvidia-logind-acl-trick-G04.conf
    fi
  fi
done
